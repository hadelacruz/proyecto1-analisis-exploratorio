...
5. ¿La edad promedio de divorcio ha disminuido con el paso del tiempo?

```{r}

divorcios_limpio %>%
  group_by(anioocu) %>%
  summarise(media_h = mean(edadhom, na.rm = TRUE), .groups = "drop") %>%
  ggplot(aes(x = factor(anioocu), y = media_h, group = 1)) +
  geom_line(color = "#c0392b", linewidth = 1.2) +
  geom_point(color = "#e67e22", size = 3) +
  labs(title = "Edad promedio del hombre por año",
       x = "Año",
       y = "Edad promedio") +
  theme_minimal() +
...
divorcios_limpio %>%
  group_by(anioocu) %>%
  summarise(media_m = mean(edadmuj, na.rm = TRUE), .groups = "drop") %>%
  ggplot(aes(x = factor(anioocu), y = media_m, group = 1)) +
  geom_line(color = "#c0392b", linewidth = 1.2) +
  geom_point(color = "#e67e22", size = 3) +
  labs(title = "Edad promedio de la mujer por año",
       x = "Año",
       y = "Edad promedio") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
