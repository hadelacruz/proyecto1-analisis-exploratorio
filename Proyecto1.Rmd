...
divorcios_sin_ign <- subset(divorcios_limpio, eschom != 9)

ggplot(divorcios_sin_ign, aes(x = factor(eschom), y = edadhom, fill = factor(eschom))) +
...
ggplot(divorcios_sin_ign, aes(x = factor(escmuj), y = edadmuj, fill = factor(escmuj))) +
  geom_boxplot(outlier.alpha = 0.2) +
  theme_minimal() +
  theme(legend.position = "none") +
...
edad_dep_hom <- aggregate(edadhom ~ depocu,
                          data = divorcios_limpio,
                          mean)

edad_dep_hom

edad_dep_muj <- aggregate(edadmuj ~ depocu,
                          data = divorcios_limpio,
                          mean)
...
colnames(edad_dep_hom)[2] <- "edad_prom"
colnames(edad_dep_muj)[2] <- "edad_prom"
